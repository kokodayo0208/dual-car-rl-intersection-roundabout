...
from env_core.road.lane import AbstractLane, LineType, StraightLane, lane_from_config
from env_core.vehicle.objects import Landmark


if TYPE_CHECKING:
    from env_core.vehicle import kinematics, objects
    ...
    def position_heading_along_route(
        ...
        def _get_route_head_with_id(route_):
            ...
            if lane_index_[2] is None:
                ...
                # ME5418 FIX: the lane id must be clamped against the number of
                # lanes of the ROUTE HEAD road, not of the vehicle's CURRENT road.
                # On a network that mixes 1-lane and 2-lane roads (e.g. an
                # intersection arm feeding a 2-lane ring) the current road may
                # have 2 lanes while the next one only has 1, which used to raise
                # "IndexError: list index out of range" a few lines below.
                n_lanes = len(self.graph[lane_index_[0]][lane_index_[1]])
                if id_ is None or id_ >= n_lanes:
                    id_ = min(id_ or 0, n_lanes - 1)
