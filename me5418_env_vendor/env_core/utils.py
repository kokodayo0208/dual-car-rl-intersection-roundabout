...
def point_in_rotated_rectangle(
    ...
    r = np.array([[c, -s], [s, c]])
    # Convert world coordinates into the rectangle's local frame.
    ru = r.T.dot(np.asarray(point) - np.asarray(center))
...
def rotated_rectangles_intersect(
    ...
    :return: do they?
    """
    # Separating-axis theorem covers edge/edge crossings as well as containment.
    # Corner/midpoint sampling alone can miss a thin oblique intersection.
    p, length_a, width_a, angle_a = rect1
    q, length_b, width_b, angle_b = rect2
    a = np.array([np.cos(angle_a), np.sin(angle_a)])
    an = np.array([-a[1], a[0]])
    b = np.array([np.cos(angle_b), np.sin(angle_b)])
    bn = np.array([-b[1], b[0]])
    delta = np.asarray(q) - np.asarray(p)
    for axis in (a, an, b, bn):
        radius_a = abs(axis.dot(a)) * length_a / 2 + abs(axis.dot(an)) * width_a / 2
        radius_b = abs(axis.dot(b)) * length_b / 2 + abs(axis.dot(bn)) * width_b / 2
        if abs(delta.dot(axis)) > radius_a + radius_b + 1e-10:
            return False
    return True
